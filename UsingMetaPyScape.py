...
api_client = metaPyScape.ApiClient(
...
try:
    ...
    api_response_intensity = featuretable_api.retrieve_intensity_matrix(
        featuretableId
    )  # of type class 'metaPyScape.models.feature_matrix.FeatureMatrix'
except ApiException as e:
...
first_sample = api_response_sample[
    0
]  # contains all 18 samples of the feature table in test project (source comparison IPB data)
...
for idx, (sid, sname) in enumerate(zip(sample_Ids, sample_names), start=1):
    msrun = {
        ...
        "scan_polarity": f"{getattr(featuretable_info, 'polarity', None).lower()} scan"
        if getattr(featuretable_info, "polarity", None)
        else None,
        "hash": None,
        "hash_method": None,
    }
...
cv_aq3_iso = Parameter(
    cv_label=None, cv_accession=None, name="isotope_pattern_aq_score", value=None
)
cv_aq4_msms = Parameter(
    cv_label=None, cv_accession=None, name="msms_aq_score", value=None
)
cv_aq5_ccs = Parameter(
    cv_label=None, cv_accession=None, name="ccs_aq_score", value=None
)
cv_aq6_mzdevi = Parameter(
    cv_label=None, cv_accession=None, name="mz_deviation", value=None
)
cv_aq7_rtdevi = Parameter(
    cv_label=None, cv_accession=None, name="rt_deviation", value=None
)
cv_aq8_isoscore = Parameter(
    cv_label=None, cv_accession=None, name="isotope_pattern_score", value=None
)
cv_aq9_msmsscore = Parameter(
    cv_label=None, cv_accession=None, name="msms_score", value=None
)
cv_aq10_ccsdevi = Parameter(
    cv_label=None, cv_accession=None, name="ccs_deviation", value=None
)
cv_aq11_annomod = Parameter(
    cv_label=None, cv_accession=None, name="annotation_modifiers", value=None
)
...
mass_list = []
mass = get_data(
    api_response_ft, mass_list, "mass"
)  # list of mass values (not used in mzTab-M)
...
chemical_name_list = []
chemical_name = get_nested_data(
    api_response_ft, chemical_name_list, "primary_annotation", "name"
)

chemical_formula_list = []
chemical_formula = get_nested_data(
    api_response_ft, chemical_formula_list, "primary_annotation", "formula"
)
...
SMF_dict = {
    ...
    "charge": ["?"]
    * 937,  # to be completed, count the + signs in adduct ion if no charge information is available in the REST API?
